export default class LinkConfig {
	...
	static getVisitsLink(lang: string): string {
		return `/${lang}/visits`;
	}
	...
	static getMetaLink(lang: string, slug: string): string {
		return `/${LinkConfig.normalizeLanguage(lang)}/meta/${slug}`;
	}

	static getSigninLink(lang: string): string {
		return `/${LinkConfig.normalizeLanguage(lang)}/signin`;
	}

	static getAuthCallbackCookieLink(lang: string): string {
		return `/${LinkConfig.normalizeLanguage(lang)}/auth/callback_cookie`;
	}

	static getAuthCallbackTokenLink(lang: string): string {
		return `/${LinkConfig.normalizeLanguage(lang)}/auth/callback_token`;
	}

	static getAuthAccountLink(lang: string): string {
		return `/${LinkConfig.normalizeLanguage(lang)}/auth/account`;
	}

	static getDashboardUrl(lang: string): string {
		return `/${LinkConfig.normalizeLanguage(lang)}/auth/dashboard`;
	}

	static getAuthErrorLink(lang: string): string {
		return `/${LinkConfig.normalizeLanguage(lang)}/auth/error`;
	}

	static getPrivacyLink(lang: string): string {
		return LinkConfig.getMetaLink(lang, "privacy");
	}

	static getTermsLink(lang: string): string {
		return LinkConfig.getMetaLink(lang, "terms");
	}

	static getAboutLink(lang: string): string {
		return LinkConfig.getMetaLink(lang, "about");
	}
